...
- validateOrderStep() — Проверяет, что введён адрес и выбран способ оплаты, показывает ошибки.
- validateContactsStep() — Проверяет корректность email и телефона; показывает ошибки.
...
#### Класс OrderFormView
Класс отвечает за управление формой заказа с двумя шагами: ввод адреса и способа оплаты, затем контактных данных. Обрабатывает обновление UI и генерацию событий.
...
- `userData:validateOrderStep` - валидация адреса и метода оплаты.
- `userData:validateContacts` - валидация контактов.
